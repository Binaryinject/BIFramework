...
namespace EmmyLua {
    ...
    class EmmyLuaService {
        ...
        private static Dictionary<Type, List<MethodInfo>> type2ExtendMethod = new();
        ...
        [MenuItem("EmmyLua/EnableService")]
        static void EnableService() {
            IsEnable = true;
            StartConnect();
        }

        [MenuItem("EmmyLua/EnableService", true)]
        static bool EnableServiceCheck() {
            return !IsEnable;
        }

        [MenuItem("EmmyLua/DisableService")]
        static void DisableService() {
            IsEnable = false;
            Stop();
        }

        [MenuItem("EmmyLua/DisableService", true)]
        ...
        private const string KEY_EMMY_SERVICE_DEBUG = "emmy.service.debug";
        private const string KEY_EMMY_DEBUG_TEXT = "emmy.debug.text";

        private static bool IsDebug {
            get { return EditorPrefs.GetBool(KEY_EMMY_SERVICE_DEBUG); }
            set { EditorPrefs.SetBool(KEY_EMMY_SERVICE_DEBUG, value); }
        }

        [MenuItem("EmmyLua/EnableDebug")]
        static void EnableDebug() {
            IsDebug = true;
            StartDebug();
        }

        [MenuItem("EmmyLua/EnableDebug", true)]
        static bool EnableDebugCheck() {
            return !IsDebug;
        }


        [MenuItem("EmmyLua/DisableDebug")]
        static void DisableDebug() {
            IsDebug = false;
        }

        [MenuItem("EmmyLua/DisableDebug", true)]
        static bool DisableDebugCheck() {
            return IsDebug;
        }

        [MenuItem("EmmyLua/DebuggerInjection")]
        static void SettingDebugCode() {
            DebuggerWindows.ShowWindow();
        }

        public class DebuggerWindows : EditorWindow {
            private static string text;

            public static void ShowWindow() {
                EditorWindow window = GetWindow(typeof(DebuggerWindows));
                window.titleContent = new GUIContent("EmmyLua Debug");
                window.Show();
                window.minSize = new Vector2(500, 150);
                window.position = new Rect(Screen.width / 2f, Screen.height / 2, 500, 150);
            }

            private void OnEnable() {
                text = EditorPrefs.GetString(KEY_EMMY_DEBUG_TEXT);
            }

            private void OnGUI() {
                GUILayout.Space(10);
                GUILayout.BeginHorizontal();
                GUILayout.Label("Injection\nCode:", GUILayout.MaxWidth(80));
                text = EditorGUILayout.TextArea(text, GUILayout.MaxHeight(110));
                ...
                if (GUILayout.Button("Store")) {
                    EditorPrefs.SetString(KEY_EMMY_DEBUG_TEXT, text);
                }
        ...
        static void StartDebug() {
            var dbg = EditorPrefs.GetString(KEY_EMMY_DEBUG_TEXT);
        ...
        private static void WriteType(BinaryWriter write, Type type) {
            if (type.IsArray) {
                ...
                if (type.IsGenericType && type.FullName != null) {
                    WriteString(write, type.FullName[..type.FullName.IndexOf('`')]);
                }
                else {
        ...
        private static void ParseExtendMethod(Type[] types) {
            ...
            try {
                foreach (var type in types) {
                    var extendsMethods = (from method in type.GetMethods(BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic)
        ...
        private static void WriteTypeData(Type type, BinaryWriter writer) {
            ...
            if (!string.IsNullOrEmpty(fullName)) {
                ...
                // base type full name
                {
                    ...
                    if (baseTypeFullName != null) {
                        if (type.BaseType.IsGenericType) {
                            baseTypeFullName = baseTypeFullName[..baseTypeFullName.IndexOf('`')];
                ...
                // methods
                var methods = (from mi in type.GetMethods(BindingFlags.Public | BindingFlags.Instance | BindingFlags.Static | BindingFlags.DeclaredOnly)
        ...
        private static Type[] GetTypes() {
            var unityTypes = from assembly in AppDomain.CurrentDomain.GetAssemblies()
                where !(assembly.ManifestModule is ModuleBuilder) && assembly.FullName != "UnityEditor"
                from type in assembly.GetExportedTypes()
                where type.BaseType != typeof(MulticastDelegate) && !type.IsInterface
                                                                 //&& !type.IsEnum
                                                                 //&& !type.IsGenericType
