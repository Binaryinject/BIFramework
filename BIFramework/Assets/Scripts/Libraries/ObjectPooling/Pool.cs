...
namespace BIFramework.Pools {
    internal sealed class Pool {
        ...
        private static readonly Dictionary<GameObject, Pool> _prefabLookup = new(64);
        private static readonly Dictionary<GameObject, Pool> _instanceLookup = new(512);

        private const int InitialSize = 128;

        private static Transform _cacheObject = null;

        private static Transform _cacheTransform {
            get {
                if (_cacheObject == null) _cacheObject = new GameObject("PoolContainer").transform;
                return _cacheObject;
            }
        }

        private static Transform _sourceObject = null;

        private static Transform _sourceTransform {
            get {
                if (_sourceObject == null) {
                    _sourceObject = new GameObject("PoolSources").transform;
                    GameObject.DontDestroyOnLoad(_sourceObject.gameObject);
                }
        ...
        public Pool(GameObject prefab) {
            ...
            if (_prototype == null) {
                _prototype = GameObject.Instantiate(prefab, _sourceTransform).AddComponent<Poolable>();
        ...
        public void Release(GameObject instance) {
            ...
            var instanceTransform = instance.transform;
            instanceTransform.SetParent(_cacheTransform);
        ...
        private Poolable CreateInstance() {
            var instance = GameObject.Instantiate(_prototype, _cacheTransform);
            var instanceGameObject = instance.gameObject;
            instance.name = $"{_prototype.name}[{_allInstances.Count}]";
