...
public partial class SROptions
{
    ...
    private bool _enableInspector = false;
    private float _AOBlend = -1;
    private static readonly int Blend = Shader.PropertyToID("_AOBlend");

    [Category("Inspector")]
    public bool EnableInspector
    {
        get { return _enableInspector; }
        set
        {
            _enableInspector = value;
            GameObject.Find("Inspector").transform.GetChild(0).gameObject.SetActive(value);
        }
    }
    [Category("Inspector")]
    [NumberRange(0, 1)]
    public float AOBlend
    {
        get {
            if (_AOBlend == -1) _AOBlend = Shader.GetGlobalFloat(Blend);
            return _AOBlend;
        }
        set {
            _AOBlend = value;
            Shader.SetGlobalFloat(Blend, _AOBlend);
        }
    }
