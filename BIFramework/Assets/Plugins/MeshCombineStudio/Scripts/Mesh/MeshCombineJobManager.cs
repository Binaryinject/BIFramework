...
namespace MeshCombineStudio {
    ...
    public class MeshCombineJobManager : MonoBehaviour {
        ...
        public void AddJob(MeshCombiner meshCombiner, MeshObjectsHolder meshObjectsHolder, Transform parent, Vector3 position) {
            ...
            // if (meshObjects.Count < 2 && meshObjects.items[0].cachedGO.mr.sharedMaterials.Length == 1 && !meshCombiner.removeTrianglesBelowSurface &&
            //     !meshCombiner.removeOverlappingTriangles && !meshCombiner.removeBackFaceTriangles) {
            ...
            for (int i = 0; i < meshObjects.Count; i++) {
                ...
                if (totalVertices + vertexCount > maxVertices) {
                    ...
                    var meshCombineJob = new MeshCombineJob(meshCombiner, meshObjectsHolder, parent, position, startIndex, length, firstMesh,
                        intersectsSurface);
        ...
        public class MeshCombineJob {
            ...
            public MeshCombineJob(MeshCombiner meshCombiner, MeshObjectsHolder meshObjectsHolder, Transform parent, Vector3 position, int startIndex,
                int length, bool firstMesh, bool intersectsSurface) {
        ...
        public class NewMeshObject {
            ...
            public void Combine(MeshCombineJob meshCombineJob) {
                ...
                for (int i = startIndex; i < endIndex; i++) {
                    ...
                    HasArray(ref newMeshCache.hasTangents, subMeshCache.hasTangents, ref newMeshCache.tangents, subMeshCache.tangents, vertexCount,
                        totalVertices, true, new Vector4(1, 1, 1, 1));
                    ...
                    HasArray(ref newMeshCache.hasColors, subMeshCache.hasColors, ref newMeshCache.colors32, subMeshCache.colors32, vertexCount, totalVertices,
                        true, new Color32(1, 1, 1, 1));
                ...
                for (int i = startIndex; i < endIndex; i++) {
                    ...
                    bool excludeBackfaceRemoval = meshCombiner.useExcludeBackfaceRemovalTag && !string.IsNullOrEmpty(meshCombiner.excludeBackfaceRemovalTag) &&
            ...
            void PrintMissingArrayWarning(MeshCombiner meshCombiner, GameObject go, Mesh mesh, string text) {
                Debug.Log("(MeshCombineStudio) => GameObject: " + go.name + " Mesh " + mesh.name + " has missing " + text +
            ...
            void HasArray<T>(ref bool hasNewArray, bool hasArray, ref T[] newArray, Array array, int vertexCount, int totalVertices,
                bool useDefaultValue = false, T defaultValue = default(T)) {
                if (hasArray) {
                    if (!hasNewArray) {
                        if (newArray == null) {
            ...
            public void CreateMesh() {
                ...
                if (meshCombineJob.intersectsSurface) {
                    if (meshCombiner.noColliders)
                        instance.camGeometryCapture.RemoveTrianglesBelowSurface(go.transform, meshCombineJob, newMeshCache, ref vertexIsBelow);
                ...
                if (meshCombiner.addMeshColliders) {
                    ...
                    if (meshCombiner.moduleColliders.Contains(meshObjectsHolder.mat)) {
                        meshCombiner.addMeshCollidersList.Add(new MeshColliderAdd(go, mesh));
                        addMeshCollider = true;
                    }
                }

                if (!addMeshCollider && meshCombiner.makeMeshesUnreadable) mesh.UploadMeshData(true);
