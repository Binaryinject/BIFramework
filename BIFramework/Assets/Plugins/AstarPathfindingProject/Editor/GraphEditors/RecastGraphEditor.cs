	...
	public class RecastGraphEditor : GraphEditor {
		...
		public override void OnInspectorGUI (NavGraph target) {
			...
			for (int i = 0; graph.meshesUnreadableAtRuntime != null && i < graph.meshesUnreadableAtRuntime.Count; i++) {
				countStillUnreadable += graph.meshesUnreadableAtRuntime[i].isReadable ? 0 : 1;
			}
			if (countStillUnreadable > 0) {
				GUILayout.BeginHorizontal();
				GUILayout.Space(EditorGUI.IndentedRect(new Rect(0, 0, 0, 0)).xMin);
				EditorGUILayout.BeginVertical(EditorStyles.helpBox);
				var c = EditorGUIUtility.IconContent("console.warnicon");
				// c.text = graph.meshesUnreadableAtRuntime.Count + " meshes will be ignored if scanned in a standalone build, because they are marked as not readable." +
				//  "If you plan to scan the graph in a standalone build, all included meshes must be marked as read/write in their import settings.";
				// meshesUnreadableAtRuntimeFoldout = EditorGUILayout.Foldout(meshesUnreadableAtRuntimeFoldout, c, true, EditorStyles.miniLabel);
				GUILayout.BeginHorizontal();
				GUILayout.BeginVertical();
				GUILayout.FlexibleSpace();
				// meshesUnreadableAtRuntimeFoldout = EditorGUILayout.Foldout(meshesUnreadableAtRuntimeFoldout);
				...
				if (meshesUnreadableAtRuntimeFoldout) {
					...
					for (int i = 0; i < graph.meshesUnreadableAtRuntime.Count; i++) {
						if (!graph.meshesUnreadableAtRuntime[i].isReadable) {
							GUILayout.BeginHorizontal();
							EditorGUI.BeginDisabledGroup(true);
							EditorGUILayout.ObjectField(graph.meshesUnreadableAtRuntime[i], typeof(Mesh), false);
							EditorGUI.EndDisabledGroup();
							if (GUILayout.Button("Make readable")) {
								var importer = ModelImporter.GetAtPath(AssetDatabase.GetAssetPath(graph.meshesUnreadableAtRuntime[i])) as ModelImporter;
								if (importer != null) {
