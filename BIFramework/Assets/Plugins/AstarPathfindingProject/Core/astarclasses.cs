...
namespace Pathfinding {
	...
	public interface IUpdatableGraph {
		...
		///
		/// May return a GraphUpdatePromise or null. If a GraphUpdatePromise is returned it will be polled on the main thread
		/// until it returns done. Only then will it progress to the next stage.
		/// The `Complete` method may be called if the graph update needs to be completed immediately.
		///
		/// See: CanUpdateAsync
		/// </summary>
		GraphUpdatePromise UpdateAreaInit(GraphUpdateObject o);
