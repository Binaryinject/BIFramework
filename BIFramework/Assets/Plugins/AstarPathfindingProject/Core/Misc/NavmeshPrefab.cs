...
namespace Pathfinding {
	...
	public class NavmeshPrefab : VersionedMonoBehaviour {
		...
		public override void DrawGizmos () {
			...
			if (!Application.isPlaying && serializedNavmesh != null) {
				...
				if (!Pathfinding.Drawing.DrawingManager.instance.gizmos.Draw(hasher)) {
					...
					var tileMeshes = RecastGraph.RecastBuilder.TileMeshes.Deserialize(serializedNavmesh.bytes);
		...
		public void SnapToClosestTileAlignment () {
			...
			if (AstarPath.active != null && AstarPath.active.data.recastGraph != null) {
				var graph = AstarPath.active.data.recastGraph;
				SnapToClosestTileAlignment(graph);
			}
		}
		...
		public void SnapToClosestTileAlignment (RecastGraph graph) {
			SnapToGraph(graph, transform.position, transform.rotation, bounds, out IntRect tileRect, out int snappedRotation, out float yOffset);
			var graphSpaceBounds = graph.GetTileBoundsInGraphSpace(tileRect);
			...
			transform.position = graph.transform.Transform(centerInGraphSpace) + transform.rotation*(-bounds.center + new Vector3(0, bounds.extents.y, 0));
		...
		public static void SnapToGraph (RecastGraph graph, Vector3 position, Quaternion rotation, Bounds bounds, out IntRect tileRect, out int snappedRotation, out float yOffset) {
			var graphRotation = Quaternion.Euler(graph.rotation);
			var rotInGraphSpace = Quaternion.Inverse(graphRotation) * (rotation * Vector3.right);
			...
			rotation = graphRotation * Quaternion.Euler(0, snappedRotation * 90, 0);
			var cornerInGraphSpace1 = graph.transform.InverseTransform(position + rotation*bounds.min);
			var cornerInGraphSpace2 = graph.transform.InverseTransform(position + rotation*bounds.max);
			var minInGraphSpace = Vector3.Min(cornerInGraphSpace1, cornerInGraphSpace2);
			var tileCoordinatesF = Vector3.Scale(minInGraphSpace, new Vector3(1.0f/graph.TileWorldSizeX, 1, 1.0f/graph.TileWorldSizeZ));
			...
			var w = Mathf.Max(1, Mathf.RoundToInt(boundsSizeInGraphSpace.x / graph.TileWorldSizeX));
			var h = Mathf.Max(1, Mathf.RoundToInt(boundsSizeInGraphSpace.y / graph.TileWorldSizeZ));
		...
		public void Apply (RecastGraph graph) {
			...
			AstarPath.active.AddWorkItem(() => {
				...
				// Make sure the graph is initialized, because the SnapToGraph function reads some properties that may
				// otherwise not be set correctly.
				graph.EnsureInitialized();
				SnapToGraph(graph, transform.position, transform.rotation, bounds, out IntRect tileRect, out int rotation, out float yOffset);
		...
		public Promise<SerializedOutput> ScanAsync (RecastGraph graph) {
			...
			var buildSettings = new RecastGraph.BuildTilesSettings(graph);
			var worldSpaceBounds = new Bounds(transform.position + transform.rotation * bounds.center, bounds.size);
			buildSettings.SetWorldSpaceBounds(worldSpaceBounds, this.transform.rotation);
		...
		public class SerializedOutput : IProgress, System.IDisposable {
			public Promise<RecastGraph.RecastBuilder.BuildTileMeshesOutput> promise;
			public byte[] data;
			public DisposeArena arena;

			public Progress Progress => promise.Progress;
		...
		struct SerializeJob : IJob {
			public Promise<RecastGraph.RecastBuilder.BuildTileMeshesOutput> tileMeshesPromise;
