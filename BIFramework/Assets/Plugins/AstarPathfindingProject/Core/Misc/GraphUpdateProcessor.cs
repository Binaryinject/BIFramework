...
namespace Pathfinding {
	...
	public interface GraphUpdatePromise {
	...
	class GraphUpdateProcessor {
		...
		class GUOSingle {
			...
			public GraphUpdatePromise graphUpdateInitPromise;
		}
			...
			while (graphUpdateQueue.Count > 0) {
				...
				foreach (IUpdatableGraph g in astar.data.GetUpdateableGraphs()) {
					...
					if (ob.nnConstraint == null || ob.nnConstraint.SuitableGraph(astar.data.GetGraphIndex(gr), gr)) {
						...
						guo.stage = GraphUpdateStage.Init;
						guo.obj = ob;
						guo.graph = g;
						ob.internalStage += 1;
		...
		bool ProcessRegularUpdates (bool force) {
			while (graphUpdateQueueRegular.Count > 0) {
				...
				// This graph update may be in progress already
				if (s.graphUpdateInitPromise != null) {
					if (force) {
						s.graphUpdateInitPromise.Complete();
					} else if (!s.graphUpdateInitPromise.IsCompleted) {
						return false;
					}
					s.graphUpdateInitPromise = null;
				}
				...
				if (s.stage == GraphUpdateStage.Init) {
					...
					s.stage = GraphUpdateStage.Main;

					if ((threading & GraphUpdateThreading.UnityInit) != 0) {
						s.graphUpdateInitPromise = s.graph.UpdateAreaInit(s.obj);
						if (s.graphUpdateInitPromise != null && !s.graphUpdateInitPromise.IsCompleted) {
							if (force) {
								s.graphUpdateInitPromise.Complete();
								s.graphUpdateInitPromise = null;
							} else {
				...
				if (s.stage == GraphUpdateStage.Main) {
