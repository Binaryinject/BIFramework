...
namespace Pathfinding {
	...
	public abstract class AIBase : VersionedMonoBehaviour {
		...
		public override void DrawGizmos () {
			...
			if (!float.IsPositiveInfinity(destination.x) && Application.isPlaying) Draw.xz.Circle(destination, 0.2f, Color.blue);
		...
		protected override int OnUpgradeSerializedData (int version, bool unityThread) {
			...
			if (version <= 2 || version == 5) rvoDensityBehavior.enabled = false;
