namespace Pathfinding.RVO {
	...
	public struct RVOQuadtreeBurst {
		...
		void Reserve (int minSize) {
			if (!boundingBoxBuffer.IsCreated) boundingBoxBuffer = new NativeArray<float3>(4, Allocator.Persistent);
		...
		public float QueryArea (float3 position, float radius) {
			if (!agents.IsCreated || numAgents == 0) return 0f;
		...
		public void DebugDraw () {
			DebugDraw(0, boundingBoxBuffer[0], boundingBoxBuffer[1]);
			for (int i = 0; i < numAgents; i++) {
				Draw.Ray(agentPositions[agents[i]], Vector3.up, Color.red);
			}
		}

		void DebugDraw (int nodeIndex, float3 nodeMin, float3 nodeMax) {
			float3 nodeMid = (nodeMin + nodeMax) * 0.5f;

			Draw.WireBox(nodeMid, nodeMax - nodeMin, Color.red);
			Draw.Line(nodeMid, nodeMax);
			...
			if ((childPointers[nodeIndex] & LeafNodeBit) != 0) {
				...
				if (endIndex == startIndex) {
					Draw.Ray(nodeMid, Vector3.up, Color.magenta);
				}
			} else {
				int childIndex = childPointers[nodeIndex];
				if (movementPlane == MovementPlane.Arbitrary) {
					DebugDraw(childIndex + 0, new float3(nodeMin.x, nodeMin.y, nodeMin.z), new float3(nodeMid.x, nodeMid.y, nodeMid.z));
					DebugDraw(childIndex + 1, new float3(nodeMin.x, nodeMin.y, nodeMid.z), new float3(nodeMid.x, nodeMid.y, nodeMax.z));
					DebugDraw(childIndex + 2, new float3(nodeMin.x, nodeMid.y, nodeMin.z), new float3(nodeMid.x, nodeMax.y, nodeMid.z));
					DebugDraw(childIndex + 3, new float3(nodeMin.x, nodeMid.y, nodeMid.z), new float3(nodeMid.x, nodeMax.y, nodeMax.z));
					DebugDraw(childIndex + 4, new float3(nodeMid.x, nodeMin.y, nodeMin.z), new float3(nodeMax.x, nodeMid.y, nodeMid.z));
					DebugDraw(childIndex + 5, new float3(nodeMid.x, nodeMin.y, nodeMid.z), new float3(nodeMax.x, nodeMid.y, nodeMax.z));
					DebugDraw(childIndex + 6, new float3(nodeMid.x, nodeMid.y, nodeMin.z), new float3(nodeMax.x, nodeMax.y, nodeMid.z));
					DebugDraw(childIndex + 7, new float3(nodeMid.x, nodeMid.y, nodeMid.z), new float3(nodeMax.x, nodeMax.y, nodeMax.z));
				} else if (movementPlane == MovementPlane.XY) {
					DebugDraw(childIndex + 0, new float3(nodeMin.x, nodeMin.y, nodeMin.z), new float3(nodeMid.x, nodeMid.y, nodeMax.z));
					DebugDraw(childIndex + 1, new float3(nodeMin.x, nodeMid.y, nodeMin.z), new float3(nodeMid.x, nodeMax.y, nodeMax.z));
					DebugDraw(childIndex + 2, new float3(nodeMid.x, nodeMin.y, nodeMin.z), new float3(nodeMax.x, nodeMid.y, nodeMax.z));
					DebugDraw(childIndex + 3, new float3(nodeMid.x, nodeMid.y, nodeMin.z), new float3(nodeMax.x, nodeMax.y, nodeMax.z));
				} else {
					DebugDraw(childIndex + 0, new float3(nodeMin.x, nodeMin.y, nodeMin.z), new float3(nodeMid.x, nodeMax.y, nodeMid.z));
					DebugDraw(childIndex + 1, new float3(nodeMin.x, nodeMin.y, nodeMid.z), new float3(nodeMid.x, nodeMax.y, nodeMax.z));
					DebugDraw(childIndex + 2, new float3(nodeMid.x, nodeMin.y, nodeMin.z), new float3(nodeMax.x, nodeMax.y, nodeMid.z));
					DebugDraw(childIndex + 3, new float3(nodeMid.x, nodeMin.y, nodeMid.z), new float3(nodeMax.x, nodeMax.y, nodeMax.z));
				}
			}
		}
	}
