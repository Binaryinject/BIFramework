...
namespace Pathfinding.RVO.Sampled {
	...
	public struct JobRVOCopy : IJob {
		...
		public void Execute () {
			...
			agentData.allowedVelocityDeviationAngles.CopyTo(agentDataCopy.allowedVelocityDeviationAngles);
			agentData.endOfPath.CopyTo(agentDataCopy.endOfPath);
			agentData.agentObstacleMapping.CopyTo(agentDataCopy.agentObstacleMapping);
	...
	public struct JobDestinationReached<MovementPlaneWrapper>: IJob where MovementPlaneWrapper : struct, IMovementPlaneWrapper {
		...
		public void Execute () {
			...
			var que = new NativeQueue<int>(Allocator.Temp);
				...
				if (effectivelyReached != output.effectivelyReachedDestination[agentIndex]) {
					...
					if (effectivelyReached == ReachedEndOfPath.Reached) {
						...
						for (int i = 0; i < count; i++) {
							...
							if (!queued[inArrow]) que.Enqueue(inArrow);
						}
					}
				}
			}
			...
			int iteration = 0;
			while (que.TryDequeue(out int agentIndex)) {
				iteration++;
	...
	public struct JobRVO<MovementPlaneWrapper> : Pathfinding.Jobs.IJobParallelForBatched where MovementPlaneWrapper : struct, IMovementPlaneWrapper {
		...
		public void ExecuteSampled (int startIndex, int batchSize) {
			...
			for (int agentIndex = startIndex; agentIndex < endIndex; agentIndex++) {
				...
				var originalDesiredVelocity = localDesiredVelocity;
		...
		/// </summary>
		void GenerateObstacleVOs (int agentIndex, NativeArray<int2> adjacentObstacleVerticesScratch, NativeArray<float> segmentDistancesScratch, NativeArray<int> sortedVerticesScratch, NativeArray<ORCALine> orcaLines, NativeArray<int> orcaLineToAgent, [NoAlias] ref int numLines, [NoAlias] in MovementPlaneWrapper movementPlane, float2 optimalVelocity) {
			var localPosition = movementPlane.ToPlane(agentData.position[agentIndex]);
			...
			var obstacleId = agentData.agentObstacleMapping[agentIndex];
			if (obstacleId == -1) return;

			var obstacleAllocations = obstacleData.obstacles[obstacleId];
			var vertices = obstacleData.obstacleVertices.GetSpan(obstacleAllocations.verticesAllocation);
			var groups = obstacleData.obstacleVertexGroups.GetSpan(obstacleAllocations.groupsAllocation);
			int vertexCount = 0;
			for (int i = 0; i < groups.Length; i++) {
				var group = groups[i];
				var startVertex = vertexCount;
				var endVertex = vertexCount + group.vertexCount - 1;
				if (endVertex >= adjacentObstacleVerticesScratch.Length) {
					// Too many vertices. Ship remaining vertices.
					break;
				}

				for (; vertexCount <= endVertex; vertexCount++) {
					sortedVerticesScratch[vertexCount] = vertexCount;
					// X coordinate is the index of the previous vertex, the y coordinate is the next vertex
					adjacentObstacleVerticesScratch[vertexCount] = new int2(vertexCount - 1, vertexCount + 1);
				}
				...
				for (int vi = startVertex; vi <= endVertex; vi++) {
					...
					int next = adjacentObstacleVerticesScratch[vi].y;
					var pos = movementPlane.ToPlane(vertex) - localPosition;
					var nextPos = movementPlane.ToPlane(vertices[next]) - localPosition;
					var dir = math.normalizesafe(nextPos - pos);
					var closestT = ClosestPointOnSegment(pos, dir, float2.zero, 0, 1);
					var dist = math.lengthsq(pos + dir*closestT);

					segmentDistancesScratch[vi] = dist;
				}
			}

			// Sort obstacle segments by distance from the agent
			sortedVerticesScratch.Slice(0, vertexCount).Sort(new SortByKey {
				keys = segmentDistancesScratch
			});
			...
			for (int i = 0; i < vertexCount; i++) {
				...
				var v3Index = adjacentObstacleVerticesScratch[v2Index].x;
				...
				var v1Position = movementPlane.ToPlane(v1) - localPosition;
				var v2Position = movementPlane.ToPlane(v2) - localPosition;
				var v3Position = movementPlane.ToPlane(v3) - localPosition;

				var segmentDir = math.normalizesafe(v2Position - v1Position);
				...
				var mn = math.min(distToCutoff, math.min(distToLeftLeg, distToRightLeg));
		...
		public void ExecuteORCA (int startIndex, int batchSize) {
			...
			NativeArray<int2> adjacentObstacleVertices = new NativeArray<int2>(SimulatorBurst.MaxObstacleVertices, Allocator.Temp, NativeArrayOptions.UninitializedMemory);
			...
			for (int agentIndex = startIndex; agentIndex < endIndex; agentIndex++) {
				...
				var debugDraw = Hint.Unlikely(agentData.debugDraw[agentIndex]);
				...
				for (int neighbourIndex = 0; neighbourIndex < neighbours.Length; neighbourIndex++) {
					...
					if (debugDraw) {
						...
						FixedString32Bytes text = $"{1/tempInverseTimeHorizon}";
						draw.Label2D(new float3(relativePosition, 0), ref text, 20);
				...
				int agentObstacleLines = numLines;
				// Obstacles are typically behind agents, so it's better to add the agent orca lines first to improve culling.
				GenerateObstacleVOs(agentIndex, adjacentObstacleVertices, segmentDistancesScratch, sortedVerticesScratch, orcaLines, orcaLineToAgent, ref numLines, in movementPlane, optimalVelocity);
				...
				bool insideAnyVO = BiasDesiredVelocity(orcaLines, agentObstacleLines, ref desiredVelocity, ref desiredTargetPointInVelocitySpace, symmetryBias);
				// If the velocity is outside all agent orca half-planes, do a more thorough check of all orca lines (including obstacles).
				insideAnyVO = insideAnyVO || DistanceInsideVOs(orcaLines, numLines, desiredVelocity) > 0;
				...
				if (!insideAnyVO && math.all(math.abs(temporaryAgentData.collisionVelocityOffsets[agentIndex]) < 0.001f)) {
					...
					if (debugDraw) {
						...
						draw.xy.Cross(movementPlane.ToWorld(localPosition + desiredTargetPointInVelocitySpace), Color.white);
					}
#endif
					...
					if (lin.firstFailedLineIndex < numLines) {
						...
						LinearProgram3D(orcaLines, numLines, 0, lin.firstFailedLineIndex, maxSpeed, ref newVelocity, scratchBuffer);
					} else {
						newVelocity = lin.velocity;
					}
					...
					if (debugDraw) {
						...
						for (int i = 0; i < numLines; i++) {
							orcaLines[i].DrawAsHalfPlane(draw, agentData.radius[agentIndex] * 5.0f, 1.0f);
						}
						draw.PopMatrix();
						draw.PopColor();
					}
					...
					if (debugDraw && forwardClearance < float.PositiveInfinity) {
		...
		struct ORCALine {
			...
			public void DrawAsHalfPlane (CommandBuilder draw, float halfPlaneLength, float halfPlaneWidth) {
				var normal = new float2(direction.y, -direction.x);
				draw.Line(new float3(point - direction*10, 0), new float3(point + direction*10, 0), Color.magenta);

				draw.SolidBox(new float3(point + normal*halfPlaneWidth*0.5f, 0), quaternion.RotateZ(math.atan2(direction.y, direction.x)), new float3(halfPlaneLength, halfPlaneWidth, 0.01f), new Color(0, 0, 0, 0.5f));
			}
		...
		/// </summary>
		static float DistanceInsideVOs (NativeArray<ORCALine> lines, int numLines, float2 velocity) {
		...
		/// </summary>
		static bool BiasDesiredVelocity (NativeArray<ORCALine> lines, int numLines, ref float2 desiredVelocity, ref float2 targetPointInVelocitySpace, float maxBiasRadians) {
			float maxDistance = DistanceInsideVOs(lines, numLines, desiredVelocity);
		...
		static void DrawVO (CommandBuilder draw, float2 circleCenter, float radius, float2 origin, Color color) {
			...
			draw.xy.Circle(new float3(circleCenter, 0), radius, alpha-delta, alpha+delta);
			...
			draw.Ray(new float3(p1, 0), new float3(math.normalizesafe(p1t)*100, 0));
			draw.Ray(new float3(p2, 0), new float3(math.normalizesafe(p2t)*100, 0));
			draw.PopColor();
#endif
		}
		...
		int GenerateNeighbourAgentVOs (int agentIndex,  NativeArray<VelocityObstacle> velocityObstacles, NativeSlice<int> neighbours) {
			...
			for (; voIndex < neighbours.Length; voIndex++) {
				...
				if (agentData.debugDraw[agentIndex]) {
					DrawVO(draw, localPosition + voBoundingOrigin * inverseAgentTimeHorizon + voCenter, totalRadius * inverseAgentTimeHorizon, localPosition + voCenter, Color.black);
