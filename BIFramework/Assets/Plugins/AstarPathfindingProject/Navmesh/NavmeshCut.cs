...
namespace Pathfinding {
	...
	using static Pathfinding.NavmeshCut;
	...
	/// [Open online documentation to see images]
	///
	/// Note that the rectangle/circle shapes are not 3D so if you rotate them you will see that the 2D shape will be rotated and then just projected down on the XZ plane.
	...
	[ExecuteAlways]
	[HelpURL("http://arongranberg.com/astar/documentation/beta/class_pathfinding_1_1_navmesh_cut.php")]
		...
		public override void DrawGizmos () {
			...
			if (selected) {
				...
				case MeshType.Sphere: {
					var sphereMatrix = Matrix4x4.TRS(tr.position, tr.rotation, Vector3.one) * Matrix4x4.Translate(center);
					var r = circleRadius * math.cmax(tr.lossyScale);
					using (Draw.WithMatrix(sphereMatrix)) {
						Draw.WireSphere(Vector3.zero, r, GizmoColor2);
					}
					break;
				}
	...
	internal static class NavmeshCutJobs {
			...
			public unsafe void Execute () {
				...
				case MeshType.Capsule: {
					...
					if (meshType == MeshType.Sphere) {
						// Spheres always use a uniform scaling
						// matrix = Matrix4x4.TRS(tr.position, tr.rotation, Vector3.one) * Matrix4x4.Translate(center);
						// r *= tr.lossyScale.magnitude;
						h = r * 2;
					} else {
						// Capsule
						h = this.height;
		...
		public static unsafe int ConvexHull (float2* points, int nPoints, float vertexMergeDistance) {
			var vertexMergeDistanceSq = vertexMergeDistance*vertexMergeDistance;
