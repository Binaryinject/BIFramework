...
namespace Pathfinding {
	...
	/// </summary>
	[HelpURL("http://arongranberg.com/astar/documentation/beta/class_pathfinding_1_1_procedural_grid_mover.php")]
	public class ProceduralGridMover : VersionedMonoBehaviour {
		...
		IEnumerator UpdateGraphCoroutine () {
			...
			var normals = graph.nodeSurfaceNormals;
			...
			Memory.Realloc(ref normalsBuffer, width*depth);
				...
				for (int l = 0; l < layers; l++) {
					...
					for (int z = 0; z < depth; z++) {
						...
						for (int x = 0; x < width; x++) {
							var bufferIndex = tz + ((x+offset.x + width) % width);
							var gridIndex = layerOffset + pz + x;
							buffer[bufferIndex] = nodes[gridIndex];
							normalsBuffer[bufferIndex] = normals[gridIndex];
						}
					}
					...
					for (int z = 0; z < depth; z++) {
						...
						for (int x = 0; x < width; x++) {
							...
							normals[layerOffset + newIndex] = normalsBuffer[newIndex];
						}
