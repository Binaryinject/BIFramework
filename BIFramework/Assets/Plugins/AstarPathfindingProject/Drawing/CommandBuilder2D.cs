...
namespace Pathfinding.Drawing {
	...
	public partial struct CommandBuilder2D {
		...
		static readonly float4x4 XZ_TO_XY_MATRIX = float4x4.RotateX(-math.PI*0.5f);
		...
		public void Circle (float2 center, float radius, float startAngle = 0f, float endAngle = 2 * math.PI) {
			Circle(new float3(center, 0), radius, startAngle, endAngle);
		}
		...
		/// <summary>\copydocref{CommandBuilder.Polyline(List<Vector2>,bool)}</summary>
		[BurstDiscard]
		...
		/// <summary>\copydocref{CommandBuilder.Polyline(Vector2[],bool)}</summary>
		[BurstDiscard]
		...
		/// <summary>\copydocref{CommandBuilder.Polyline(float2[],bool)}</summary>
		[BurstDiscard]
		...
		/// <summary>\copydocref{CommandBuilder.Polyline(NativeArray<float2>,bool)}</summary>
