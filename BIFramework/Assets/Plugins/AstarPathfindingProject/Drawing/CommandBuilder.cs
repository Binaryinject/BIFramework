...
namespace Pathfinding.Drawing {
	...
	public partial struct CommandBuilder : IDisposable {
		...
		internal unsafe void Add<T>(T value) where T : struct {
			...
			unsafe {
#if ENABLE_UNITY_COLLECTIONS_CHECKS
				UnityEngine.Assertions.Assert.IsTrue(bufferSize + num <= buffer->Capacity);
#endif
...
#endif
				unsafe {
					new CommandBuilder(buffer, gizmos, 0, uniqueID).PopMatrix();
					buffer = null;
				}
			}
		}
		...
		public struct ScopeColor : IDisposable {
			...
			public void Dispose () {
				...
				unsafe {
					...
					new CommandBuilder(buffer, gizmos, 0, uniqueID).PopColor();
					buffer = null;
				}
			}
		}
...
#endif
				unsafe {
					new CommandBuilder(buffer, gizmos, 0, uniqueID).PopDuration();
					buffer = null;
				}
			}
		}
...
#endif
				unsafe {
					new CommandBuilder(buffer, gizmos, 0, uniqueID).PopLineWidth();
					buffer = null;
				}
			}
		}
		...
		/// </code>
		///
		/// See: <see cref="CircleXZ(float3,float,float,float)"/>
		...
		/// </code>
		///
		/// See: <see cref="SolidCircleXZ(float3,float,float,float)"/>
		/// See: <see cref="SolidCircleXY(float3,float,float,float)"/>
		/// </summary>
		/// <param name="center">Center of the imaginary circle that the arc is part of.</param>
		/// <param name="start">Starting point of the arc.</param>
		/// <param name="end">End point of the arc.</param>
		...
		/// [Open online documentation to see images]
		///
		/// See: <see cref="WirePlane(float3,quaternion,float2)"/>
		...
		/// [Open online documentation to see images]
		///
		/// This is identical to <see cref="WirePlane(float3,quaternion,float2)"/>, but this name is added for consistency.
		///
		/// See: <see cref="WirePolygon"/>
		/// </summary>
		...
		/// </summary>
		[System.Obsolete("Use Darw.xy.SolidRectangle instead")]
