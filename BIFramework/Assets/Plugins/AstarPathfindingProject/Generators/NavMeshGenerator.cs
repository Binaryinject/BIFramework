...
namespace Pathfinding {
	...
	public class NavMeshGraph : NavmeshBase, IUpdatableGraph {
		...
		protected override IEnumerable<Progress> ScanInternal (bool async) {
			...
			var meshDatas = Mesh.AcquireReadOnlyMeshData(this.sourceMesh);
			Pathfinding.Util.MeshUtility.GetMeshData(meshDatas, 0, out var vertices, out var indices);
			meshDatas.Dispose();
			Profiler.EndSample();
			...
			var promise = JobBuildNavmesh.Schedule(vertices, indices, meshToGraphSpace);

			if (async) {
				while (!promise.IsCompleted) {
					System.Threading.Thread.Sleep(1);
					yield return promise.Progress;
				}
			}

			var navmeshOutput = promise.Complete();
			navmeshOutput.ToManaged(out var verticesInGraphSpace, out var compressedIndices);
			...
			ReplaceTile(0, 0, verticesInGraphSpace, compressedIndices);
		...
		struct JobBuildNavmesh : IJob {
			...
			public struct BuildNavmeshOutput : IProgress, System.IDisposable {
				// Length of 2, the vertices are in the first buffer, the indices in the second buffer
				public NativeArray<UnsafeAppendBuffer> buffers;

				public Progress Progress => new Progress(0.0f, "Building Nodes");

				public void Dispose () {
					buffers.Dispose();
				}

				public void ToManaged (out Int3[] vertices, out int[] indices) {
					vertices = Memory.UnsafeAppendBufferToArray<Int3>(buffers[0]);
					indices = Memory.UnsafeAppendBufferToArray<int>(buffers[1]);
				}
			}

			public static Promise<BuildNavmeshOutput> Schedule (NativeArray<Vector3> vertices, NativeArray<int> indices, Matrix4x4 meshToGraph) {
				var outputBuffers = new NativeArray<UnsafeAppendBuffer>(2, Allocator.Persistent);

				var job = new JobBuildNavmesh {
					vertices = vertices,
					indices = indices,
			...
			public void Execute () {
				...
				unsafe {
					var ptr = (UnsafeAppendBuffer*)this.outputBuffers.GetUnsafePtr();
					var outputVertices = ptr;
					var outputTriangles = ptr+1;
