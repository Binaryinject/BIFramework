...
namespace Pathfinding {
	...
	public class TriangleMeshNode : MeshNode {
		...
		public bool GetPortal (GraphNode toNode, System.Collections.Generic.List<Vector3> left, System.Collections.Generic.List<Vector3> right, bool backwards, out int aIndex, out int bIndex) {
			...
			// A connection was found, but it didn't use an edge.
			// This is likely a custom connection.
			...
			if (tileIndex1 != tileIndex2) {
				...
				if (otherEdge != -1) {
					...
					var t1 = VectorMath.ClosestPointOnLineFactor(v1a, v1b, v2a);
					var t2 = VectorMath.ClosestPointOnLineFactor(v1a, v1b, v2b);
					var v1aClamped = v1a + (v1b - v1a) * Mathf.Clamp01(Mathf.Min(t1, t2));
					var v1bClamped = v1a + (v1b - v1a) * Mathf.Clamp01(Mathf.Max(t1, t2));
					v1a = v1aClamped;
					v1b = v1bClamped;
				}
			}
